...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==================== HARD GAME RESET (frontend-wide) ====================
  // Some "Salir" buttons live in navigation.js (and others in game.js/insp.js).
  // To ensure leaving the game never leaks state into the next match, always call this.
  function hardResetGame(reason) {
    try {
      if (typeof window.resetGameCompletely === 'function') {
        window.resetGameCompletely(reason || 'hardResetGame');
      } else {
        // Fallback: at least force re-init on next enter
        try { window.gameInitialized = false; } catch (e) {}
        // Clear common globals used by game.js
        try { delete window.currentPlayers; } catch (e) {}
        try { delete window.currentGameMode; } catch (e) {}
        try { delete window.currentLocalPlayerIndex; } catch (e) {}
        try { delete window.QuantumMusOnlineRoom; } catch (e) {}
        try { window.onlineMode = false; } catch (e) {}
      }
    } catch (e) {
      console.warn('[hardResetGame] failed:', e);
      try { window.gameInitialized = false; } catch (e2) {}
    }
  }
  ...
  function initializeLobby() {
    ...
    leaveLobbyButton.onclick = () => {
      emitLeaveRoom();
      // If we are leaving after having played, wipe all match state.
      hardResetGame('leave-lobby-button');
      showScreen('menu');
      resetGameState();
    };
  ...
  // Si estás en online y quieres que el servidor devuelva a lobby a todos:
        if (gameState.socket && gameState.socket.connected && gameState.roomId) {
          gameState.socket.emit('return_to_lobby', { room_id: gameState.roomId });
        }

        resetMatchKeepPlayerName();

        // Re-monta el lobby con el mismo nombre, pero sin estado previo
        showScreen('lobby');
        initializeLobby();
      }
  ...
  if (playAgainButton) {
    playAgainButton.addEventListener('click', () => {
      // New game must not reuse any previous match state
      hardResetGame('play-again');
      showScreen('lobby');
      // Keep lobby/player identity, but force fresh game init next time
  ...
  function resetGameForNewRound() {
    // Ensure a totally fresh game state
    hardResetGame('resetGameForNewRound');
  ...
  // Reset de partida pero manteniendo el nombre del jugador
  function resetMatchKeepPlayerName() {
    // Limpia juego (timers, gameState de game.js, globals, etc.)
    hardResetGame('resetMatchKeepPlayerName');

    // Mantener identidad, borrar SOLO datos de partida/lobby anterior
    gameState.selectedCharacter = null;
    gameState.players = [{
      name: gameState.playerName,
      character: null,
    ...
    // En local/demo, esto evita que se quede “enganchado”
    gameState.demoMode = false;

    // Opcional: si quieres mantener sala online, NO toques roomId/roomCode.
    // Si quieres que al salir de partida se “reinicie” la sala local, descomenta:
