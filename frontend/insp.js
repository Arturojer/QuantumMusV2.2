...
function getNotificationContainer() {
  ...
  function showOrdagoWinner(winningTeam) {
    ...
    ordagoExitBtn.addEventListener('click', () => {
      ...
      // Reset game state completely (if available)
      if (typeof window.resetGameCompletely === 'function') {
        window.resetGameCompletely('ordago-exit');
      }
      // Fallback: reset init flag
    ...
    for (let i = 0; i < 4; i++) {
      ...
      cards.forEach((card, cardIndex) => {
        ...
        if (i === localPlayerIndex) {
          ...
          // Only initialize selection state if not already set
