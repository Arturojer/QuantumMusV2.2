...
function lockGameInteractions(reason = '') {
  ...
  try { console.log('[LOCK]', reason || ''); } catch (e) {}
}

function unlockGameInteractions(reason = '') {
  gameState.interactionLocked = false;
  try { console.log('[UNLOCK]', reason || ''); } catch (e) {}
}
...
const gameState = {
  ...
  ,
    // When true, player inputs/AI/timers should pause (e.g., end-of-hand summary)
...
// ===================== FULL RESET (leave match) =====================
// When leaving the match, we must wipe ALL state so a new game starts clean.
function resetGameCompletely(reason = 'reset') {
  try { lockGameInteractions('resetGameCompletely:' + reason); } catch (e) {}
  // Clear global timers/timeouts
  try { if (timerInterval) { clearTimeout(timerInterval); timerInterval = null; } } catch (e) {}
  try { if (aiDecisionTimeout) { clearTimeout(aiDecisionTimeout); aiDecisionTimeout = null; } } catch (e) {}
  try {
    if (gameState.autoDiscardTimeouts) {
      Object.keys(gameState.autoDiscardTimeouts).forEach(k => { try { clearTimeout(gameState.autoDiscardTimeouts[k]); } catch (e) {} });
      gameState.autoDiscardTimeouts = {};
    }
  } catch (e) {}

  // Reset core state (mutate in place because gameState is const)
  try {
    ...
    // Reset entanglement bookkeeping
    gameState.entanglement = {
      pairs: [],
      events: [],
      statistics: { total_pairs: 0, activated_pairs: 0, superposition_pairs: 0, game_mode: (window.currentGameMode || '4'), pairs_per_team: 2 },
      playerEntanglements: {}
    };

    // Locks/guards
    gameState.interactionLocked = false;
    gameState.playerTimeoutConsumed = [false, false, false, false];
    gameState._puntoStartGuard = false;

    // Reset tunneling flags
    gameState.forceTunnelNextRound = false;
    gameState.forceTunnelNextHand = false;
  } catch (e) {
    console.warn('[resetGameCompletely] state reset failed', e);
  }

  // Remove transient UI
  try {
    document.querySelectorAll('.game-over-modal, .ordago-winner-banner, .hand-summary-modal, .character-modal, .auto-declaration-message')
  ...
  // Ensure game can be re-initialized
  try { gameInitialized = false; } catch (e) {}
  try { unlockGameInteractions('resetGameCompletely:' + reason); } catch (e) {}
}
// Make available to navigation.js or other UI
...
function initGame() {
      ...
      try {
        ...
        if (penalty && penalty.penalized) {
          ...
          if (penalty.points_deducted) {
            ...
            gameState.teams[playerTeam].score = Math.max(0, (gameState.teams[playerTeam].score || 0) - penalty.points_deducted);
  ...
  function startDiscardPhase() {
    ...
    // Reset auto-bot discard guard for this discard phase
    gameState._autoBotsDiscardedThisPhase = false;
  ...
  function handleDiscard(playerIndex, cardIndices) {
    ...
    // If the local player has just discarded in MUS (offline/local), auto-discard for all bots
    // so we don't have to wait for their timers.
    try {
      ...
      if (!isOnlineGame && gameState.currentRound === 'MUS' && gameState.waitingForDiscard && playerIndex === localIdx) {
        if (!gameState._autoBotsDiscardedThisPhase) {
          gameState._autoBotsDiscardedThisPhase = true;

          // Clear any per-AI autodiscard fallback timers (we are resolving immediately)
          if (gameState.autoDiscardTimeouts) {
            Object.keys(gameState.autoDiscardTimeouts).forEach(k => {
              try { clearTimeout(gameState.autoDiscardTimeouts[k]); } catch (e) {}
              try { delete gameState.autoDiscardTimeouts[k]; } catch (e) {}
            });
          }

          for (let i = 0; i < 4; i++) {
            if (i === localIdx) continue;
            if (gameState.cardsDiscarded[i]) continue;
            // Simple policy: bots discard all cards immediately
            setTimeout(() => {
              try { handleDiscard(i, [0, 1, 2, 3]); } catch (e) {}
            }, 120 + (i * 60));
          }
        }
      }
    ...
    // (intentionally no further waits: bots will discard via the block above)
  ...
  function shouldAutoDeclarePares(playerIndex) {
    ...
    // If we can't reliably read the 4 cards from the DOM (rare UI race),
    // auto-declare to avoid blocking the declaration phase.
    if (cardElements.length !== 4) return true;

    // If there is no unresolved quantum state (entangled/superposed & not collapsed),
    // we can always auto-declare.
    const hasUnresolvedQuantum = cardElements.some(el =>
      el.dataset.collapsed !== 'true' && (el.dataset.entangled === 'true' || el.dataset.superposed === 'true')
    );
    if (!hasUnresolvedQuantum) return true;

    // If there IS unresolved quantum state, only auto-declare when the outcome is certain
    // across all possible collapses (e.g., classical pairs already guarantee pares, or
    // no possible collapse yields pares).
  ...
  function getAutoParesDeclaration(playerIndex) {
    ...
    // Fallback: if DOM doesn't show 4 cards, decide based on the current known hand.
    if (cardElements.length !== 4) {
      try {
        const cards = getPlayerCards(playerIndex);
        return calculatePares(cards) !== null;
      } catch (e) {
        return false;
      }
    }

    // Build per-card possible value sets.
    // - Collapsed: single value
    // - Entangled (unresolved): [main, partner]
    // - Superposed (unresolved): [value, superposedValue]
    // - Classical: single value
    const baseCards = getPlayerCards(playerIndex);
    const valueOptionsByIndex = cardElements.map((el, idx) => {
      const isCollapsed = el.dataset.collapsed === 'true';
      const v = baseCards[idx]?.value || el.dataset.value || el.dataset.mainValue;
      if (isCollapsed) return [v];

      if (el.dataset.entangled === 'true') {
        const mainValue = el.dataset.mainValue || el.dataset.value;
        const partnerValue = el.dataset.partner;
        // If partnerValue is missing for any reason, fall back to single value
        if (!partnerValue) return [mainValue || v];
        return Array.from(new Set([mainValue || v, partnerValue]));
      }

      if (el.dataset.superposed === 'true') {
        const v1 = el.dataset.value || el.dataset.mainValue || v;
        const v2 = el.dataset.superposedValue;
        if (!v2) return [v1];
    ...
    // Enumerate all combinations (<= 2^4 = 16) and determine if pares is deterministic.
    let canHavePares = false;
    let canNotHavePares = false;

    const buildCardsForCombo = (choiceIdxs) => {
      return baseCards.map((card, idx) => ({
        value: valueOptionsByIndex[idx][choiceIdxs[idx] || 0],
        suit: card.suit
      }));
    };

    const dfs = (pos, choiceIdxs) => {
      if (pos === valueOptionsByIndex.length) {
        const testCards = buildCardsForCombo(choiceIdxs);
        const result = calculatePares(testCards);
        if (result !== null) canHavePares = true;
        else canNotHavePares = true;
        return;
      }
      const opts = valueOptionsByIndex[pos];
      for (let i = 0; i < opts.length; i++) {
        choiceIdxs[pos] = i;
        dfs(pos + 1, choiceIdxs);
    ...
    if (canHavePares && !canNotHavePares) return true;
    if (!canHavePares && canNotHavePares) return false;
    return null; // uncertain -> player must choose (TENGO vs PUEDE)
  }
  ...
  function autoDeclareJuego(playerIndex) {
    ...
    // If the hand has no active quantum effects (no uncollapsed entanglement/superposition),
    // the outcome is deterministic and MUST be auto-declared (no player choice).
    const hasUncollapsedEntanglement = Array.from(cardElements).some(el =>
      el && el.dataset && el.dataset.entangled === 'true' && el.dataset.collapsed !== 'true'
    );
    const hasUncollapsedSuperposed = Array.from(cardElements).some(el =>
      el && el.dataset && el.dataset.superposed === 'true' && el.dataset.collapsed !== 'true'
    );
    const hasAnyQuantum = hasUncollapsedEntanglement || hasUncollapsedSuperposed;

    if (!hasAnyQuantum) {
      const juegoResult = calculateJuego(cards);
      const hasJuego = juegoResult.hasJuego;
      console.log(`Player ${playerIndex + 1} auto-declared JUEGO: ${hasJuego} (fully collapsed hand - no quantum cards)`);
      return hasJuego;
    }

    // Get UNCOLLAPSED entangled card indices and their possible values (collapsed entanglement shouldn't block determinism)
    ...
    cardElements.forEach((cardEl, idx) => {
      if (cardEl && cardEl.dataset && cardEl.dataset.entangled === 'true' && cardEl.dataset.collapsed !== 'true') {
    ...
    // If there are superposed (uncollapsed) cards but NO entanglement, the outcome isn't deterministic.
    // In that case, allow manual declaration (including 'puede').
    if (!hasEntangled && hasUncollapsedSuperposed) {
      console.log(`Player ${playerIndex + 1} auto-declared JUEGO: null (uncollapsed superposition present)`);
      return null;
    }

    if (!hasEntangled) {
      // No uncollapsed entangled cards - deterministic (even if some cards were entangled in the past but have collapsed)
      const juegoResult = calculateJuego(cards);
      const hasJuego = juegoResult.hasJuego;
      console.log(`Player ${playerIndex + 1} auto-declared JUEGO: ${hasJuego} (deterministic hand)`);
      return hasJuego;
    }
  ...
  function proceedWithJuegoDeclaration() {
    ...
    // If autoResult is still unknown, but the hand is already deterministic (no superposition/entanglement unresolved),
    // then declare automatically based on the actual (collapsed) values.
    if (autoResult === null) {
      try {
        const playerId = `player${playerIdx + 1}`;
        const cardEls = Array.from(document.querySelectorAll(`#${playerId}-zone .quantum-card`));
        const gameMode = window.currentGameMode || '8';

        const getCardPoints = (v) => {
          if (v === 'A') return 1;
          if (v === '2') return gameMode === '4' ? 2 : 1;
          if (v === '3') return gameMode === '4' ? 3 : 10;
          if (v === 'J' || v === 'Q' || v === 'K') return 10;
          return parseInt(v) || 0;
        };

        const isDeterministicCard = (el) => {
          // Deterministic if already collapsed OR it's not in an unresolved quantum state.
          if (!el) return false;
          if (el.dataset.collapsed === 'true') return true;
          if (el.dataset.entangled === 'true') return false;
          if (el.dataset.superposed === 'true') return false;
          return true;
        };

        const allDeterministic = (cardEls.length === 4) && cardEls.every(isDeterministicCard);

        if (allDeterministic) {
          const values = cardEls.map(el => (el.dataset.value || el.dataset.mainValue || el.dataset.originalValue || '').trim());
          const sum = values.reduce((acc, v) => acc + getCardPoints(v), 0);
          autoResult = (sum >= 31);
          console.log(`[AUTO-JUEGO] Player ${playerIdx + 1}: deterministic hand, sum=${sum} -> auto-declaring ${autoResult ? 'TENGO' : 'NO TENGO'}`);
        }
      } catch (e) {
        console.warn('[AUTO-JUEGO] deterministic check failed', e);
      }
    }

    if (autoResult !== null) {
      // Can auto-declare (short delay to match insp.js pacing).
      // IMPORTANT: if the hand is fully determined, do NOT let the local player choose manually.
      const localPlayerIndex = window.currentLocalPlayerIndex ?? 0;
      if (playerIdx === localPlayerIndex) {
        try {
          const buttons = document.querySelectorAll('.scoreboard-controls .quantum-gate');
          buttons.forEach(btn => { try { btn.disabled = true; } catch(e){} });
        } catch (e) { /* ignore */ }
      }
  ...
  function showDeclarationBanner(roundName, cb) {
    ...
    setTimeout(() => {
      banner.style.opacity = '0'; banner.style.transform = 'scale(0.9)';
  ...
  function showOrdagoWinner(winningTeam) {
    ...
    ordagoExitBtn.addEventListener('click', () => {
      ...
      // Full reset so the next match starts clean
      try { resetGameCompletely('exit-to-lobby'); } catch (e) {}
  ...
  function finishHand() {
    lockGameInteractions('finishHand');
    console.log('Finishing hand - awarding points via CONTEO');

    // Show a lightweight banner while we wait to present CONTEO details.
    showCalculatingPointsBanner();
  ...
  function showHandSummary(scoringDetails = {}, opts = {}) {
    const team1Score = gameState.teams.team1.score;
    const team2Score = gameState.teams.team2.score;
    ...
    modal.innerHTML = `
      ...
      ">
        ...
        <p style="color: var(--circuit-blueprint); font-size: 1rem; margin-bottom: 18px;">
          Puntuaciones finales de la mano.
        </p>
        <button id="hand-summary-continue" style="
    ...
    const btn = modal.querySelector('#hand-summary-continue');
    const continueNow = () => {
      try { closeModal(modal); } catch (e) {}
      try { if (opts && typeof opts.onContinue === 'function') opts.onContinue(); } catch (e) {}
    };

    if (btn) {
      btn.addEventListener('click', () => {
        try { btn.style.transform = 'scale(0.98)'; setTimeout(() => { btn.style.transform = 'scale(1)'; }, 80); } catch (e) {}
        continueNow();
      });
    }

    // Also allow Enter/Space to continue while modal is open
    const keyHandler = (ev) => {
    ...
    try { window.addEventListener('keydown', keyHandler, { once: true }); } catch (e) {}

    }
  
  // Start new hand
  function startNewHand() {
    unlockGameInteractions('startNewHand');
  ...
  function updateButtonStates(isLocalPlayerTurn) {
    ...
    const isBettingRound = ['GRANDE', 'CHICA', 'PARES', 'JUEGO', 'PUNTO'].includes(gameState.currentRound);
    ...
    if (musBtn) {
      // MUS/TENGO button:
      // - visible in MUS round
      // - visible in PARES/JUEGO ONLY during the declaration sub-phase (before all declarations are made)
      const paresDeclPending = (gameState.currentRound === 'PARES' && gameState.paresDeclarations && Object.keys(gameState.paresDeclarations).length < 4);
      const juegoDeclPending = (gameState.currentRound === 'JUEGO' && gameState.juegoDeclarations && Object.keys(gameState.juegoDeclarations).length < 4);
      musBtn.style.display = (isMus || paresDeclPending || juegoDeclPending) ? '' : 'none';
    }
    
    // PASO button:
    // - Hidden during the initial MUS round (requested: remove the PASO button there)
    // - Visible elsewhere (betting rounds) and repurposed as "PUEDE" during declaration phases
    if (pasoBtn) {
      const paresDeclPending = (gameState.currentRound === 'PARES' && gameState.paresDeclarations && Object.keys(gameState.paresDeclarations).length < 4);
      const juegoDeclPending = (gameState.currentRound === 'JUEGO' && gameState.juegoDeclarations && Object.keys(gameState.juegoDeclarations).length < 4);
      const isDeclarationPhase = paresDeclPending || juegoDeclPending;
      pasoBtn.style.display = (isMus && !isDeclarationPhase) ? 'none' : '';
    }
  ...
  // -------------------------------------------------------------------
  // AI helpers (offline/local only)
  // The game features superposed and entangled cards. For AI heuristics we
  // treat each non-collapsed quantum card as a small set of possible values
  // (usually 2) with equal weight. This keeps the AI lightweight while still
  // reacting to superposition/entanglement.
  const AI_CONFIG = {
    // "cooldown" / thinking delay to make play feel natural
    thinkDelayMinMs: 900,
    thinkDelayMaxMs: 1800,
    // Faster responses for simple acknowledge/accept flows
    fastDelayMinMs: 550,
    fastDelayMaxMs: 1100,
    // How aggressive the AI is when it feels strong
    aggression: 0.55
  };

  // Extra pause specifically for AI bets/raises so bots don't chain-envido too fast.
  function aiBetExtraCooldownMs(action) {
    return (action === 'raise' || action === 'envido' || action === 'ordago')
      ? aiRandInt(320, 680)
      : 0;
  }

  function aiRandInt(min, max) {
    return Math.floor(min + Math.random() * (max - min + 1));
  }

  function aiDelay(kind = 'normal') {
    if (kind === 'fast') return aiRandInt(AI_CONFIG.fastDelayMinMs, AI_CONFIG.fastDelayMaxMs);
    return aiRandInt(AI_CONFIG.thinkDelayMinMs, AI_CONFIG.thinkDelayMaxMs);
  }

  // Extra cooldown for aggressive actions so AI-vs-AI betting doesn't feel instant.
  function aiBettingExtraDelayMs(action) {
    if (action === 'raise' || action === 'envido' || action === 'ordago') {
      return aiRandInt(320, 680);
    }
    return 0;
  }

  function aiNormalizeCardValue(v) {
    if (v === undefined || v === null) return '';
    const s = String(v).trim();
    // Values are already A,J,Q,K or numbers in the UI
    return s;
  }

  // Ranking for GRANDE (higher is better)
  function aiGrandeRank(v) {
    const val = aiNormalizeCardValue(v);
    const map = { K: 12, Q: 11, J: 10, '7': 7, '6': 6, '5': 5, '4': 4, '3': 3, '2': 2, A: 1, '1': 1 };
    return map[val] ?? parseInt(val, 10) ?? 0;
  }

  // Ranking for CHICA (lower is better -> return inverted so higher is better)
  function aiChicaRank(v) {
    const r = aiGrandeRank(v);
    // invert within [1..12]
    return 13 - r;
  }

  // Point value for JUEGO/PUNTO (A=1, 2-7 pip, figures=10)
  function aiTantoValue(v) {
    const val = aiNormalizeCardValue(v);
    if (val === 'K' || val === 'Q' || val === 'J') return 10;
    if (val === 'A' || val === '1') return 1;
    const n = parseInt(val, 10);
    return Number.isFinite(n) ? n : 0;
  }

  function aiGetPlayerCardEls(playerIndex) {
    const playerId = `player${playerIndex + 1}`;
    return Array.from(document.querySelectorAll(`#${playerId}-zone .quantum-card`));
  }

  // Read visible / quantum information from the DOM in a way consistent with scoreboard.
  function aiReadHandInfo(playerIndex) {
    const cardEls = aiGetPlayerCardEls(playerIndex);
    const cardValues = [];
    const entangledInfo = [];
    const superposedInfo = [];
    const outcomes = []; // per card: [{value, p}, ...]

    cardEls.forEach((card, idx) => {
      const isCollapsed = card.dataset.collapsed === 'true';
      const decoration = card.querySelector('.quantum-decoration');
      let v1 = decoration ? decoration.textContent.trim() : (card.dataset.value || card.dataset.mainValue || '');
      v1 = aiNormalizeCardValue(v1);

      if (isCollapsed) {
        const cardTop = card.querySelector('.card-top');
        if (cardTop && cardTop.textContent.includes('⟩')) {
          const match = cardTop.textContent.match(/\|(\w+)⟩/);
          if (match) v1 = aiNormalizeCardValue(match[1]);
        }
      }

      cardValues.push(v1);

      // Possible outcomes (for AI heuristic only)
      let outs = [{ value: v1, p: 1 }];
      if (!isCollapsed) {
        if (card.dataset.entangled === 'true' && card.dataset.partner) {
          const v2 = aiNormalizeCardValue(card.dataset.partner);
          outs = (v2 && v2 !== v1) ? [{ value: v1, p: 0.5 }, { value: v2, p: 0.5 }] : [{ value: v1, p: 1 }];
          entangledInfo.push({ index: idx, value1: v1, value2: v2 });
        } else if (card.dataset.superposed === 'true' && card.dataset.superposedValue) {
          const v2 = aiNormalizeCardValue(card.dataset.superposedValue);
          outs = (v2 && v2 !== v1) ? [{ value: v1, p: 0.5 }, { value: v2, p: 0.5 }] : [{ value: v1, p: 1 }];
          superposedInfo.push({ index: idx, value1: v1, value2: v2 });
        }
      }
      outcomes.push(outs);
    });
    ...
    const gameMode = window.currentGameMode || '8';
    const paresProb = parseInt(calculateParesProbability(cardValues, entangledInfo, superposedInfo), 10) || 0;
    const juegoProb = parseInt(calculateJuegoProbability(cardValues, entangledInfo, gameMode), 10) || 0;

    // Expected ranks (cheap approximations for GRANDE/CHICA)
    const expGrande = outcomes.reduce((acc, outs) => acc + outs.reduce((a, o) => a + o.p * aiGrandeRank(o.value), 0), 0);
    const expChica = outcomes.reduce((acc, outs) => acc + outs.reduce((a, o) => a + o.p * aiChicaRank(o.value), 0), 0);

    // Expected tanto for PUNTO/JUEGO
    const expTanto = outcomes.reduce((acc, outs) => acc + outs.reduce((a, o) => a + o.p * aiTantoValue(o.value), 0), 0);

    return { cardEls, cardValues, entangledInfo, superposedInfo, outcomes, paresProb, juegoProb, expGrande, expChica, expTanto };
  }

  function aiOpponentCloseToWin() {
    const t1 = gameState.teams?.team1?.score ?? 0;
    const t2 = gameState.teams?.team2?.score ?? 0;
    // "close" ~ within 5 points of 40
    return (t1 >= 35 || t2 >= 35);
  }

  // Decide whether to CUT mus by raising/envidar (in this UI, envido from MUS ends mus).
  function aiShouldCutMus(playerIndex, info) {
    // Strong signals: high chance of juego or pares, or very strong grande
    const strongJuego = info.juegoProb >= 65;
    const strongPares = info.paresProb >= 60;
    const strongGrande = info.expGrande >= 34; // heuristic threshold

    // If opponent close to win, cut more often to avoid giving free re-draws.
    const urgency = aiOpponentCloseToWin();

    let pCut = 0;
    if (strongJuego) pCut += 0.45;
    if (strongPares) pCut += 0.30;
    if (strongGrande) pCut += 0.20;
    if (urgency) pCut += 0.20;

    // Add a little randomness so play isn't deterministic
    pCut *= (0.85 + 0.3 * Math.random());
    return Math.random() < Math.min(0.85, Math.max(0.05, pCut));
  }

  // Pick a small MUS envido amount (cuts mus). Keep it modest to feel human.
  function aiChooseMusEnvidoAmount(info) {
    // If very strong, bet a bit more.
    if (info.juegoProb >= 80 || info.paresProb >= 75) return 5;
    if (info.juegoProb >= 70 || info.paresProb >= 65) return 3;
    return 2;
  }

  // Decide which cards to discard during the mus discard phase.
  // NOTE: We can't simulate future draws exactly, so we keep the best synergy
  // for GRANDE/CHICA + pares/juego probabilities and discard the rest.
  function aiChooseDiscardIndices(playerIndex, info) {
    // Never discard collapsed entangled cards (already resolved) - but can discard any.
    // Score each card by how valuable it is to keep.
    const baseKeepScore = (outs) => {
      const eg = outs.reduce((a, o) => a + o.p * aiGrandeRank(o.value), 0);
      const ec = outs.reduce((a, o) => a + o.p * aiChicaRank(o.value), 0);
      const et = outs.reduce((a, o) => a + o.p * aiTantoValue(o.value), 0);
      // Grande slightly more important early, but keep balance.
      return 0.9 * eg + 0.5 * ec + 0.35 * et;
    };

    // Pair synergy: if a card shares a possible value with another card, boost both.
    const possibleVals = info.outcomes.map(outs => new Set(outs.map(o => o.value)));
    const pairBoost = new Array(info.outcomes.length).fill(0);
    for (let i = 0; i < possibleVals.length; i++) {
      for (let j = i + 1; j < possibleVals.length; j++) {
        const inter = [...possibleVals[i]].some(v => possibleVals[j].has(v));
        if (inter) {
          pairBoost[i] += 4;
          pairBoost[j] += 4;
        }
    ...
    const keepScores = info.outcomes.map((outs, i) => baseKeepScore(outs) + pairBoost[i]);

    // Decide how many to discard:
    // - If we already look strong in pares/juego, discard fewer.
    // - If weak, discard more to seek improvement.
    let targetKeep = 4;
    if (info.juegoProb < 35 && info.paresProb < 35) targetKeep = 1;
    else if (info.juegoProb < 45 && info.paresProb < 45) targetKeep = 2;
    else if (info.juegoProb < 60 && info.paresProb < 55) targetKeep = 3;

    // Sort indices by keepScore desc; keep best targetKeep
    const idxs = keepScores.map((s, i) => ({ i, s })).sort((a, b) => b.s - a.s).map(o => o.i);
    const keepSet = new Set(idxs.slice(0, targetKeep));
    const discard = [];
    for (let i = 0; i < 4; i++) if (!keepSet.has(i)) discard.push(i);

    // Small chance to keep all ("me planto") when already strong
    if (discard.length > 0 && (info.juegoProb >= 70 || info.paresProb >= 65) && Math.random() < 0.25) {
      return [];
    }
    return discard;
  }

  function aiStrength01ForRound(round, info) {
    // Returns a rough [0..1] strength signal used for betting decisions.
    switch (round) {
      case 'GRANDE':
        // expGrande max ~ 48 (4*K). Threshold around 30-34.
        return Math.min(1, Math.max(0, (info.expGrande - 26) / 16));
      case 'CHICA':
        // expChica max ~ 48 (4*A inverted). Similar scaling.
        return Math.min(1, Math.max(0, (info.expChica - 26) / 16));
      case 'PARES':
        return Math.min(1, Math.max(0, info.paresProb / 100));
      case 'JUEGO':
        return Math.min(1, Math.max(0, info.juegoProb / 100));
      case 'PUNTO':
        // Expected tanto in [4..40]. When there's no juego, 25-30 is decent.
        return Math.min(1, Math.max(0, (info.expTanto - 18) / 14));
      default:
        return 0.5;
    }
  }
  ...
  if (gameState.interactionLocked) {
    console.log('[AI] Skipping decision: interactions locked');
    return;
  }
    ...
    // Natural "cooldown" so AIs feel less robotic (and respects the request
    // for ~0.5s-1s per AI action). If a bet is pending, respond a little faster.
    const responsesNow = gameState.currentBet && gameState.currentBet.responses;
    const hasPendingBetNow = !!(gameState.currentBet && gameState.currentBet.bettingTeam && !responsesNow?.[playerIndex]);
    const baseDelay = aiDelay(hasPendingBetNow ? 'fast' : 'normal');
    aiDecisionTimeout = setTimeout(() => {
      ...
      // Helper: add a small additional delay for aggressive betting actions
      // (ENVIDO/RAISE/ORDAGO) so bot-vs-bot doesn't chain too fast.
      const aiDoBettingAction = (action, amount = 0) => {
        const extra = aiBettingExtraDelayMs(action);
        if (extra > 0) {
          // Prevent the player's timer from expiring while we wait the extra cooldown
          if (timerInterval) { clearTimeout(timerInterval); timerInterval = null; }
          hideTimerBar(playerIndex);
          setTimeout(() => {
            if (gameState.interactionLocked) return;
            if (gameState.activePlayerIndex !== playerIndex) return;
            handleBettingRound(playerIndex, action, amount);
          }, extra);
        } else {
          handleBettingRound(playerIndex, action, amount);
        }
      ...
      if (gameState.waitingForDiscard) {
        const info = aiReadHandInfo(playerIndex);
        const discardIdx = aiChooseDiscardIndices(playerIndex, info);
        console.log(`[AI] Player ${playerIndex + 1} discarding indices:`, discardIdx);
        handleDiscard(playerIndex, discardIdx);
      } else if (gameState.currentRound === 'MUS') {
        // First MUS speaking round: only MUS or ENVIDO (to cut mus).
        // (Rule reminder: no PASO/ORDAGO here for the AI.)
        const info = aiReadHandInfo(playerIndex);
        const cut = aiShouldCutMus(playerIndex, info);
        if (cut) {
          const amount = aiChooseMusEnvidoAmount(info);
          console.log(`[AI] Player ${playerIndex + 1} cuts MUS with ENVIDO (${amount})`);
          handleMusRound(playerIndex, 'envido', { amount });
        } else {
          console.log(`[AI] Player ${playerIndex + 1} says MUS`);
          handleMusRound(playerIndex, 'mus');
        }
      } else if (['GRANDE', 'CHICA', 'PARES', 'JUEGO', 'PUNTO'].includes(gameState.currentRound)) {
        // Betting rounds: respond based on a rough hand-strength estimate.
        // This is still lightweight, but noticeably less trivial than always accept/paso.
        const info = aiReadHandInfo(playerIndex);
        ...
        const betAmount = gameState.currentBet?.amount ?? 0;

        // Round-specific strength signal
        let strength = 0.5;
        if (gameState.currentRound === 'GRANDE') strength = Math.min(1, info.expGrande / 44);
        else if (gameState.currentRound === 'CHICA') strength = Math.min(1, info.expChica / 44);
        else if (gameState.currentRound === 'PARES') strength = Math.min(1, info.paresProb / 100);
        else if (gameState.currentRound === 'JUEGO') strength = Math.min(1, info.juegoProb / 100);
        else if (gameState.currentRound === 'PUNTO') strength = Math.min(1, info.expTanto / 32);

        // Accept threshold scales with bet size (bigger bet => need more confidence)
        const acceptTh = Math.min(0.85, 0.45 + 0.02 * betAmount);
        const raiseTh = Math.min(0.90, acceptTh + 0.20);

        if (hasPendingBet && gameState.currentBet.bettingTeam && !responses[playerIndex]) {
          if (strength >= acceptTh) {
            // Sometimes re-raise if very strong (but keep it modest)
            if (strength >= raiseTh && Math.random() < AI_CONFIG.aggression) {
              const raiseTo = Math.max(betAmount + 2, betAmount + 1);
              console.log(`[AI] Player ${playerIndex + 1} re-raises in ${gameState.currentRound} to ${raiseTo} (strength ${strength.toFixed(2)})`);
              aiDoBettingAction('raise', raiseTo);
            } else {
              console.log(`[AI] Player ${playerIndex + 1} accepts in ${gameState.currentRound} (strength ${strength.toFixed(2)})`);
              aiDoBettingAction('accept');
            }
          } else {
            console.log(`[AI] Player ${playerIndex + 1} PASO in ${gameState.currentRound} (strength ${strength.toFixed(2)})`);
            aiDoBettingAction('paso');
          }
        } else {
          // No pending bet. Occasionally open with a small raise when strong.
          if (strength >= 0.75 && Math.random() < (AI_CONFIG.aggression * 0.6)) {
            const open = 2 + (strength > 0.9 ? 3 : 0);
            console.log(`[AI] Player ${playerIndex + 1} opens with raise ${open} in ${gameState.currentRound} (strength ${strength.toFixed(2)})`);
            aiDoBettingAction('raise', open);
          } else {
    ...
    }, baseDelay); // AI thinking delay (ms)
  }
  ...
  function updateScoreboard() {
    ...
    if (buttons.length >= 5) {
      ...
      if (gameState.currentRound === 'CONTEO') {
        ...
        // In MUS round - show MUS, ENVIDO, ÓRDAGO (no PASO button)
        ...
        // "NO TENGO" is not selectable in declaración: solo TENGO o PUEDE.
        buttons[1].style.display = 'none';
        ...
        if (button3Label) button3Label.textContent = 'PUEDE';
        musButton.style.display = 'inline-flex'; // Show TENGO button
        buttons[1].style.display = 'none'; // Hide NO TENGO (auto/timeout handles 'no' when certain)
        buttons[2].style.display = 'inline-flex'; // Show PUEDE button
        acceptButton.style.display = 'none'; // Hide ACCEPT during declaration
        buttons[4].style.display = 'none'; // Hide ÓRDAGO during declaration
  ...
  // Banner shown briefly during CONTEO delay.
  function showCalculatingPointsBanner() {
    try {
      if (gameState._calculatingPointsBanner && gameState._calculatingPointsBanner.remove) {
        gameState._calculatingPointsBanner.remove();
      }
    } catch (e) {}

    const banner = document.createElement('div');
    banner.className = 'calculating-points-banner';
    banner.style.cssText = `
      position: fixed;
      top: 20px;
      left: 50%;
    ...
    `;
    banner.textContent = 'Calculando puntos...';

    const container = getNotificationContainer ? getNotificationContainer() : document.body;
    container.appendChild(banner);
    setTimeout(() => { banner.style.opacity = '1'; }, 10);

    gameState._calculatingPointsBanner = banner;
  }

  function hideCalculatingPointsBanner() {
    try {
      const banner = gameState._calculatingPointsBanner;
      if (!banner) return;
      banner.style.opacity = '0';
      setTimeout(() => {
        try { banner.remove(); } catch (e) {}
      }, 300);
      gameState._calculatingPointsBanner = null;
    } catch (e) {}
  }
 ...
 // END OF initGame()
  ...
  function checkPredictionPenalty(playerIndex, roundName, declaration) {
    ...
    if (predictionWrong) {
      ...
      const playerTeam = getPlayerTeam(playerIndex);
      gameState.teams[playerTeam].score = Math.max(0, (gameState.teams[playerTeam].score || 0) - penalty);
  ...
  /**
 * Show penalty notification with graphic
 * - Uses the shared notification stack (vertical) instead of a fixed centered modal
 * - Briefly pauses game flow (~0.9s) for a more natural feel
 */
function showPenaltyNotification(playerIndex, roundName, penalty) {
  const wasLocked = !!(gameState && gameState.interactionLocked);
  // Briefly pause interactions so the notification feels "weighted" and doesn't overlap rapid state changes
  if (!wasLocked) { try { lockGameInteractions('penalty-notification'); } catch (e) {} }

  const notification = document.createElement('div');
  notification.className = 'penalty-notification';
  notification.innerHTML = `
    <div class="penalty-icon" style="font-size: 2rem; margin-bottom: 6px;">⚠️</div>
    <div class="penalty-text">
      <strong style="font-size: 1.1rem; color: #ff4444;">Penalización</strong><br>
      <span style="font-size: 1rem; margin: 6px 0; display: block;">Jugador ${playerIndex + 1} · ${roundName}</span>
      <span style="color: #ff4444; font-size: 1.4rem; font-weight: bold;">-${penalty} Punto${penalty > 1 ? 's' : ''}</span>
    </div>
  `;

  // NOTE: Do NOT use fixed positioning here. The notification-stack already handles vertical stacking.
  notification.style.cssText = `
    position: relative;
    transform: scale(0);
    background: linear-gradient(135deg, rgba(30, 41, 59, 0.98), rgba(15, 23, 42, 0.98));
    ...
    padding: 14px 18px;
    text-align: center;
    box-shadow: 0 12px 30px rgba(0, 0, 0, 0.45), 0 0 18px rgba(255, 68, 68, 0.25);
    color: white;
  ...
  const STYLE_ID = 'penalty-notification-style';
  if (!document.getElementById(STYLE_ID)) {
    ...
    style.textContent = `
      ...
      @keyframes penalty-disappear {
        0%   { transform: translateY(0px)  scale(1.00); opacity: 1; }
        100% { transform: translateY(-6px) scale(0.92); opacity: 0; }
      }
    `;
    document.head.appendChild(style);
  }

  // Animate in (stack-friendly)
  setTimeout(() => {
    notification.style.animation = 'penalty-appear 0.25s ease-out forwards';
  }, 10);

  // Keep it on screen a bit longer since it's now a banner (but still unobtrusive)
  const visibleMs = 1700;
  const pauseMs = 900;

  // Resume game flow shortly after showing (so it doesn't feel frozen too long)
  
  setTimeout(() => {
    if (!wasLocked) { try { unlockGameInteractions('penalty-notification'); } catch (e) {} }
  }, pauseMs);

  // Remove after visibleMs
  setTimeout(() => {
    notification.style.animation = 'penalty-disappear 0.25s ease-in forwards';
    setTimeout(() => {
